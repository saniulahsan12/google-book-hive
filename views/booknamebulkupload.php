...
function google_book_hive_book_name_bulk_upload() {
    ...
    if( isset($_POST['upload-excel']) && wp_verify_nonce( $_POST['file-nonce'], 'upload-nonce' ) && !empty($_FILES['file-data']) ):
        ...
        if( !empty( get_user_meta(get_current_user_id(), 'isbn_file', true) ) ):
            $result_processed = google_book_hive_extract_and_upload_product_criteria_2( get_user_meta(get_current_user_id(), 'isbn_file', true) );
        ...
        <div class="form-area">
            ...
            <form action="" method="post" enctype="multipart/form-data">
				...
				<div class="form-group">
                    <label for="fileupload">Choose Excel/CSV File by Book Name</label>
					<input type="file" class="form-control" id="fileupload" name="file-data">
				</div>
                <?php wp_nonce_field('upload-nonce', 'file-nonce'); ?>
                <input id="submit" type="submit" name="upload-excel" class="btn btn-primary btn-block" value="Upload Excel">
            </form>
